...
const publishBlog = async (req, res) => {
  try {
    console.log('📝 PublishBlog called by user:', req.user?.email || req.user?._id);
    const { fileId, title, excerpt, tags, metaTitle, metaDescription, featuredImage } = req.body;
    console.log('📦 Request data:', { fileId, title, excerpt: excerpt?.substring(0, 50), tags });

    // Validate required fields
    if (!fileId || !title || title.trim() === '') {
      console.log('❌ Validation failed: Missing fileId or title');
      return res.status(400).json({ error: 'File ID and title are required' });
    }

    // Get the file content
    console.log('🔍 Looking up file:', fileId, 'for user:', req.user._id);
    ...
    console.log('💾 Saving blog with slug:', blog.slug);
    await blog.save();
    console.log('✅ Blog saved successfully:', blog._id);

    res.status(201).json(blog);
  } catch (error) {
    console.error('❌ Publish blog error:', error.name, error.message);
